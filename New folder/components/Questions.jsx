            ...
            <ul className='flex flex-col' key={questions?._id}>
                {
                    options?.map((q, i) => (
                        <li key={i}>
                            ...
                            <label className='mt-px pl-[0.15rem] hover:cursor-pointer flex items-center gap-4 p-4 rounded-xl bg-white bg-opacity-90 backdrop-blur-2xl shadow-xl hover:bg-opacity-75 peer-checked:bg-purple-900 peer-checked:text-white cursor-pointer transition' htmlFor={`q${i}-option`}>{q}</label>
                            <div className={`w-40 h-6 ${result[trace] == q ? 'bg-green-600' : 'bg-red-500'}`}></div>
                        </li>
                    ))
                }
            </ul>
